...
dword _pascal QoiExport(ExportFrame *frame)
{
    QoiError gstat = QE_NO_ERROR;
    ...
    gstat = QoiExportBitmapFHandle(frame->EF_transferVMFile, bmblock, frame->EF_outputFile);

    // free bmblock
    VMFreeVMChain(frame->EF_transferVMFile, VMCHAIN_MAKE_FROM_VM_BLOCK(bmblock));

    if(gstat != QE_NO_ERROR)
    {
        // error code translation
        switch(gstat)
        {
            case QE_BLOCK_LOCK_FAILURE:
            case QE_OUT_OF_MEMORY:
                return(TE_OUT_OF_MEMORY);

            case QE_INVALID_FILE:
