...
main(int argc, char **argv)
{
    ...
    pixels = (dword)info.width * info.height;
    result = WebPImportBegin(&source, (void *)0, &decoder,
                             &bitmap, &info, pixels);
    assert(result == WEBP_RESULT_OK);
    assert(decoder != NullHandle && bitmap != NullHandle);
    WebPImportDestroy(decoder);
    free(bitmap->pixelsP);
    free(bitmap);
    ...
    if (pixels > 1) {
        result = WebPImportBegin(&source, (void *)0, &decoder,
                                 &bitmap, &info, pixels - 1);
        assert(result == WEBP_ERROR_DEFERRED);
        assert(decoder == NullHandle && bitmap == NullHandle);
        assert((dword)info.width * info.height == pixels);
    }
    free(dataP);
