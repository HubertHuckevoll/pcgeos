...
The final maxPixels argument is zero for an unrestricted import. For a valid
image above a nonzero limit, Begin returns WEBP_ERROR_DEFERRED after parsing
the header but before allocating decode buffers or a bitmap. The decoder and
bitmap outputs remain NullHandle, and info contains the image dimensions.
The decoder's separate 2048-pixel dimension limit is an ordinary error.
