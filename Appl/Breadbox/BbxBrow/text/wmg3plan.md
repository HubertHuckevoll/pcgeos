...
The crucial thing is that **the pixel protection happens inside the real
importer**. Streamed GIF/JPEG imports apply it as soon as their decoder knows
the dimensions; download-first imports apply it from the completed file.
...
For an unknown-length size-limited request, disable progressive loading from the network stream before receiving the body: it might cross the byte cap after partial pixels were already published. A known final Content-Length that passed the pre-body limit check may retain its loading-progress callback and stream normally.

Do not disable post-download local-file import progress. When streaming is unavailable, after WMG3HTTP returns the completed file and the import-time pixel admission accepts it, the existing import-progress callback and waiting-image layout path must display supported formats incrementally while decoding.
